...
const prepareAsync = (onSuccess: () => void, onError: (err: any) => void) => getAvplay()?.prepareAsync(onSuccess, onError);
const setDisplayRect = (x: number, y: number, w: number, h: number) => getAvplay()?.setDisplayRect(x, y, w, h);
